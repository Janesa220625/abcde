  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Always create a mock admin user to bypass login
        console.log("Creating default mock user to bypass login");
        ...
        // Save mock user to Supabase storage
        await saveToStorage("dev-storage/mock-user.json", mockUser);
      } catch (err) {
        console.error("Error setting up auto user:", err);
        setError("Failed to set up auto user");
      } finally {
        setIsLoading(false);
      }
    };

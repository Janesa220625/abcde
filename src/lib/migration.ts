 ...
 */
export const migrateFirebaseToSupabase = async (): Promise<{
  success: boolean;
  migrated: string[];
  errors: string[];
  details?: Record<string, any>;
  ...
  try {
    ...
    // 1. Migrate users from Firebase
    try {
      console.log("Starting migration of Firebase users...");
      const usersMigrated = await migrateFirebaseUsers();
      if (usersMigrated) {
        migrated.push("Firebase users");
        details.firebaseUsersMigration = "Success";
      }
    } catch (error) {
      console.error("Error migrating Firebase users:", error);
      errors.push("Firebase users");
      details.firebaseUsersMigration =
        error instanceof Error ? error.message : String(error);
    }

    // 2. Migrate other data types as needed
    // Add additional migration steps here for products, inventory, etc.
    ...
    return {
      ...
      errors: [
        ...
        error instanceof Error
          ? error.message
          : "Unknown error during Firebase migration",
      ],
      details,
    };
  }
...
/**
 * Migrate users from Firebase to Supabase profiles table
 */
async function migrateFirebaseUsers(): Promise<boolean> {
  try {
    ...
    console.log(`Found ${usersSnapshot.size} users to migrate from Firebase`);

    // For each Firebase user, check if they exist in Supabase and create if not
    for (const doc of usersSnapshot.docs) {
      const userData = doc.data();
      console.log(`Processing user migration for: ${userData.email || doc.id}`);
      ...
      if (existingProfile) {
        console.log(
          `User ${userData.email || doc.id} already exists in Supabase, skipping`,
        );
        continue;
      }

      // Map Firebase user data to Supabase profile format
      const profileData = {
        id: doc.id, // Use Firebase document ID as the profile ID
        email: userData.email || "",
        first_name: userData.firstName || userData.first_name || "",
        last_name: userData.lastName || userData.last_name || "",
        role: userData.role || "viewer",
        warehouse_id: userData.warehouseId || userData.warehouse_id || null,
        created_at:
      ...
      if (insertError) {
        console.error(
          `Error migrating user ${userData.email || doc.id}:`,
          insertError,
        );
      } else {
        console.log(
          `Successfully migrated user ${userData.email || doc.id} to Supabase:`,
          insertData,
        );
      }
    }
    ...
    console.error("Error in migrateFirebaseUsers:", error);
    throw error;
  }
}
